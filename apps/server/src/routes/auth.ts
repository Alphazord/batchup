...
/**
 * Derives the cookie domain from WEB_APP_URL so the session cookie
 * is accessible from both the server (api.batchup.fun) and the web app (batchup.fun).
 * Returns undefined for localhost (let the browser use the default exact-host scope).
 */
function getCookieDomain(webAppUrl: string | undefined): string | undefined {
  ...
  try {
    ...
    // "batchup.fun" → ".batchup.fun"  (leading dot makes it apply to all subdomains)
...
function generateUsername(email: string): string {
  ...
  const sanitized = prefix
    ...
    .replace(/^[^a-z]/, "")
    .slice(0, 20);
  return `${sanitized}${Math.random().toString(36).slice(2, 6)}`;
}
...
authRoutes.get("/google/callback", async (c) => {
  ...
  try {
    ...
    const cookieDomain = getCookieDomain(webUrl);
...
authRoutes.get("/me", async (c) => {
  ...
  if (remaining < SESSION_MAX_AGE / 2) {
    ...
    if (deleteResult.meta?.changes !== 0) {
      ...
      const isRefreshSecure = new URL(c.req.url).protocol === "https:";
      const refreshCookieDomain = getCookieDomain(c.env.WEB_APP_URL);
      setCookie(c, "session", newSessionToken, {
        httpOnly: true,
        secure: isRefreshSecure,
        sameSite: "Lax",
        path: "/",
        maxAge: SESSION_MAX_AGE,
...
authRoutes.post("/logout", async (c) => {
  ...
  const isLogoutSecure = new URL(c.req.url).protocol === "https:";
  const logoutCookieDomain = getCookieDomain(c.env.WEB_APP_URL);
  setCookie(c, "session", "", {
    httpOnly: true,
    secure: isLogoutSecure,
    sameSite: "Lax",
    maxAge: 0,
    path: "/",
